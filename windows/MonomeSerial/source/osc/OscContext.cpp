...
extern "C" int regcomp(regex_t *preg, const char *regex, int cflags);
extern "C" int regexec(const regex_t *preg, const char *string, size_t nmatch,
					   regmatch_t pmatch[], int eflags);

//extern "C" int snprintf(char *, size_t, const char *, /*args*/ ...);
extern "C" int portable_snprintf(char *str, size_t str_m, const char *fmt, /*args*/ ...);
...
OscContext::convertValueStringToAtom(OscAtom *atom, const string& atomic_value_string)
{
    ...
    endptr = 0;
    atom->setValue((float)strtod(atomic_value_string.c_str(), &endptr));
...
OscContext::convertAtomToValueString(char *valueString, size_t len, OscAtom *atom)
{
    switch (atom->type()) {
    case OscAtom::kOscAtomTypeInt:
        portable_snprintf(valueString, len, "%d", atom->valueAsInt());
        break;

    case OscAtom::kOscAtomTypeFloat:
        portable_snprintf(valueString, len, "%f", atom->valueAsFloat());
        break;

    case OscAtom::kOscAtomTypeString:
        portable_snprintf(valueString, len, "%s", atom->valueAsCString());
        break;
...
OscContext::_atomize(const string& atomic_value_string)
{
    ...
    do {
        ...
        endptr = 0;
        atom->setValue((float)strtol(atomic_value_string.c_str(), &endptr, 0));
